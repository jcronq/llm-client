...
class ExperimentRunner:
    def __init__(self, experiment_config_file: Path):
        ...
        self.llm = ChatOpenAI(openai_api_key=CFG.openai_api_key, model=self.model, temperature=self.temperature)
    ...
    def generate_query_prompt(self, query: Optional[str] = None) -> list[BaseMessage]:
        if query is None:
            query = self.query
        messages = []
    ...
    def run_experiment(self):
        ...
        for perturbation in self.perturbations:
            ...
            if query is not None:
                ...
                result = self.llm.predict_messages(prompt).content
